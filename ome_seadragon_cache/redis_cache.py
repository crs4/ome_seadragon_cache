...
class RedisCache(CacheInterface):
    ...
    def _get_tile_key(self, image_id, level, column, row, tile_size, image_format, limit_bounds,
                      image_quality=None, rendering_engine='UNKNOWN'):
        return super(RedisCache, self)._get_tile_key(image_id, level, column, row,
                                                     tile_size, image_format, limit_bounds,
    ...
    def tile_to_cache(self, image_id, image_obj, level, column, row, tile_size, image_format, limit_bounds,
                      image_quality=None, rendering_engine='UNKNOWN'):
        out_buffer = BytesIO()
        image_obj.save(out_buffer, image_format)
        tile_key = self._get_tile_key(image_id, level, column, row, tile_size, image_format.upper(),
                                      limit_bounds, image_quality, rendering_engine)
        self.client.set(tile_key, out_buffer.getvalue())
        self._set_expire_time(tile_key)

    def tile_from_cache(self, image_id, level, column, row, tile_size, image_format, limit_bounds,
                        image_quality=None, rendering_engine='UNKNOWN'):
        tile_key = self._get_tile_key(image_id, level, column, row, tile_size, image_format.upper(),
                                      limit_bounds, image_quality, rendering_engine)
        self.logger.debug('Tile from cache: %s' % tile_key)
        tile_str = self.client.get(tile_key)
        if tile_str:
            self.logger.debug('Tile retrieved')
